from app.shared.request_tools.json_encode import JsonEncode
from app.shared.request_tools.multipart_encode import MultipartEncode
from app.shared.request_tools.encode_factory import EncodeFactory
from app.shared.request_tools.encode import Encode
...
def process_video(data: dict, video: bytes, callback, *args) -> dict:
    with tempfile.NamedTemporaryFile(suffix='.mp4') as temp:
        ...
        with tempfile.NamedTemporaryFile(suffix='.mp4',) as temp_out:
            ...
            encode = 'MP4V'
            out: cv.VideoWriter = cv.VideoWriter(temp_out.name, cv.VideoWriter_fourcc(*encode), fps, (width, height))

            out_data: dict = {'video': {'encode': encode}}
...
def encode_h264(video: bytes) -> dict:
    with tempfile.NamedTemporaryFile(suffix='.mp4') as temp:
        temp.write(video)
        with tempfile.NamedTemporaryFile(suffix='.mp4',) as temp_out_264:
                os.system(f"ffmpeg -y -i {temp.name} -vcodec libx264 {temp_out_264.name}")
                return temp_out_264.file.read()